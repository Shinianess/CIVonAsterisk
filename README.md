...
This project implements a **CIV** (Caller ID Verification) scheme to prevent Caller ID spoofing. It uses a challenge‑response mechanism to verify whether the caller is the owner of the claimed caller ID. The callee's endpoint (stock SIP client) receives either `Verified Caller ID` or `Unverified Caller ID` as the verification result.
...
| `verify_result` | Final string: `Verified Caller ID` or `Spoofed Caller ID`. |
